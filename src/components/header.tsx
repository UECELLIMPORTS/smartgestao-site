  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b backdrop-blur-md"
        style={{ background: 'rgba(255,255,255,.85)', borderColor: 'var(--border)' }}>
        <div className="container-x flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href={BRAND.appUrl}
              className="hidden md:inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors hover:text-slate-900"
            ...
            <button
              ...
              className="md:hidden flex h-10 w-10 items-center justify-center rounded-lg border transition-colors hover:bg-slate-50"
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
              {NAV.map(n => (
                <Link key={n.href} href={n.href} onClick={() => setOpen(false)}
                  className="block rounded-lg px-4 py-3 text-sm font-medium transition-colors hover:bg-slate-50"
              ...
              <a href={BRAND.appUrl} onClick={() => setOpen(false)}
                className="mt-2 flex items-center justify-center gap-2 rounded-lg border px-4 py-3 text-sm font-medium transition-colors hover:bg-slate-50"
                style={{ borderColor: 'var(--border)', color: 'var(--muted)' }}>
                Acessar minha conta
              </a>
              <a href={whatsappLink('Oi! Vim do site, gostaria de saber mais.')}
                target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)}
                className="mt-2 flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium border transition-colors hover:bg-slate-50"
